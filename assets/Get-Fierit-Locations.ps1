...
try {
    ...
    $splatParams = @{
        Uri     = "$($config.BaseUrl)/rootdata/locations"
        Method  = 'GET'
        Headers = $headers    }
    $responseLocations = Invoke-RestMethod @splatParams -UseBasicParsing

    Write-Output $responseLocations
